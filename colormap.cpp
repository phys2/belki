...
QPixmap apply(cv::Mat &source, double scale)
{
	...
	return apply(matrixB);
}

QPixmap apply(cv::Mat1b &source)
{
	cv::Mat3b colorMatrix;
	cv::applyColorMap(source, colorMatrix, colormap::magma);

	return QPixmap::fromImage({colorMatrix.data, colorMatrix.cols, colorMatrix.rows,
	                           (int)colorMatrix.step, QImage::Format_RGB888});
}

/* Magma map from https://github.com/BIDS/colormap/blob/master/colormaps.py */
std::array<cv::Vec3b, 256> magma = {{
    {0, 0, 4},
    {1, 0, 5},
    {1, 1, 6},
    {1, 1, 8},
